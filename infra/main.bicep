...
module network 'modules/network.bicep' = {
  ...
  dependsOn: [rg]
  params: {
    name: 'vnet-forevergames-${environment}'
    location: location
    tags: tags
    addressSpace: '10.20.0.0/16'
    containerAppsSubnetPrefix: '10.20.0.0/23'
    postgresSubnetPrefix: '10.20.2.0/24'
    privateEndpointsSubnetPrefix: '10.20.3.0/24'
  }
}
module dnsPrivate 'modules/private-dns.bicep' = {
  scope: resourceGroup(resourceGroupName)
  name: 'private-dns'
  dependsOn: [rg]
  params: { vnetId: network.outputs.id, environment: environment }
}
...
module monitoring 'modules/monitoring.bicep' = {
  ...
  name: 'monitoring'
  dependsOn: [rg]
  params: { environment: environment, location: location, tags: tags, retentionDays: 30 }
}
module vault 'modules/key-vault.bicep' = {
  ...
  params: {
    ...
    purgeProtection: environment == 'prod'
    privateEndpointsSubnetId: network.outputs.privateEndpointsSubnetId
    privateDnsZoneId: dnsPrivate.outputs.vaultZoneId
  }
}
module postgres 'modules/postgresql.bicep' = {
  ...
  params: {
    ...
    privateDnsZoneId: dnsPrivate.outputs.postgresZoneId
...
module alerts 'modules/alerts.bicep' = {
  ...
  name: 'alerts'
  dependsOn: [rg]
  params: { environment: environment, tags: tags, postgresId: postgres.outputs.id, contactEmails: budgetContactEmails }
}
...
module migrationJob 'modules/migration-job.bicep' = {
  ...
  params: {
    ...
    image: '${registry.outputs.loginServer}/forever-games-migrations:${imageTag}'
...
module app 'modules/container-app.bicep' = if (deployApplication) {
  ...
  params: {
    ...
    tags: tags
    environmentId: appEnvironment.outputs.id
    image: '${registry.outputs.loginServer}/forever-games-web:${imageTag}'
    registryServer: registry.outputs.loginServer
    keyVaultUri: vault.outputs.uri
...
module publicDns 'modules/dns.bicep' = if (deployApplication && configureDns) {
  scope: resourceGroup(dnsZoneResourceGroup)
  name: 'public-dns'
  params: { zoneName: dnsZoneName, containerAppFqdn: app!.outputs.fqdn, verificationId: dnsVerificationId }
}
module budget 'modules/budget.bicep' = {
  name: 'budget'
  params: { budgetName: 'budget-forevergames-${environment}', amount: budgetAmount, startDate: budgetStartDate, contactEmails: budgetContactEmails }
}
